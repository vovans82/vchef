...
bash "build gecode from source" do
  ...
  environment ({'CC' => '/usr/bin/gcc44', 'CXX' => '/usr/bin/g++44'})
  code <<-EOH
...
# Without this chef-server will fail to start
execute "local lib added to ldconfig" do
  command "echo '/usr/local/lib' >> /etc/ld.so.conf && /sbin/ldconfig"
  action :run
