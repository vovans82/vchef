...
template "#{node[:nginx][:dir]}/sites-available/chef" do
  source "nginx_chef.conf.erb"
  owner "root"
  group "root"
  mode 0644
end 
...
template "#{node[:nginx][:dir]}/sites-available/chef-ssl" do
  source "nginx_chef-ssl.conf.erb"
  owner "root"
  group "root"
  mode 0644
end 

nginx_site "chef-ssl" do
  enable true
end
